...
<section class="content flex flex-col items-center pt-32 px-5 sm:px-20 -z-10 ">
    <form class="flex items-center w-full border-2 rounded-lg">
        <div class="relative w-full">
            ...
            <input class="h-10 w-full pl-10 pr-3 text-center outline-none rounded-lg" type="text" name="query"
                placeholder="Cari Rumah Sakit..." value="<?= $query ?>">
            <?php if ($query) : ?>
    ...
    <section class="flex">
        ...
        <div class="font-bold mt-20">Rumah sakit tidak ditemukan.</div>
        <?php else : ?>
        <div class="hospital_list grid grid-flow-row grid-cols-1  lg:grid-cols-2 xl:grid-cols-3 gap-5 my-10">
            <?php while ($row = mysqli_fetch_assoc($result)) : ?>
            <div
                class="hospital_card flex flex-col justify-between border-2 border-gray-200 rounded-lg items-start bg-white p-3 sm:p-5">
